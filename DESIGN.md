...
## Documentation and notices revision

Telemetry retains every original heading, sentence, and list item in its original order. Its desktop composition uses a narrow section-heading column and a readable body column, collapsing to one natural reading column on mobile. Ruled rows distinguish counted actions; a quieter bullet list presents exclusions. The IP-address/anonymous-terminology caveat remains intact and is separated by a restrained left rule. No counters, dashboard widgets, animation, or new privacy claims are introduced.

Full-page not-found, unavailable, incomplete-link, invalid-management-link, malformed-private-envelope, and generic request-error states use a shared NoticePage presentation. Existing headings, messages, status/alert roles, and recovery links are passed through unchanged. A small visit-seeded sigil is decorative and hidden from assistive technology. Inline validation, password forms, loading states, and deleted-success behavior are untouched.

Homepage, creation, shared global styles, routes, API, cryptography, and telemetry logic are unchanged by this revision. Validation: all 355 tests, lint, and production build pass. Chromium checks compare rendered documentation text verbatim with the previous version, confirm no documentation API calls, check 320–1440 px layouts and preserved error live-region roles, and exercise not-found keyboard recovery. Rendered desktop/mobile screenshots were visually inspected. Backend errors in browser checks were mocked; no live backend was used.
